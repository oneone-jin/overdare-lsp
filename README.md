...
If your workspace contains a `*.ovdrjm` project file (OVERDARE Studio's project format),
the language server detects it automatically and keeps a `sourcemap.json` in sync with it —
no manual sourcemap generator to configure. The instance tree, including OVERDARE's flat
`Lua/` script export folder (with `Name`, `Name_1`, `Name_2`, ... disambiguating same-named
sibling scripts), is resolved from the live `.ovdrjm` file on every Studio save/play/publish.
The server listens to changes of the `sourcemap.json` file present at the workspace root; it's
recommended to add this file to your `.gitignore`.

If you don't have a `.ovdrjm` file (e.g. non-OVERDARE Luau/Roblox projects), sourcemap
generation falls back to the original Rojo-based flow: running
`rojo sourcemap --watch default.project.json --output sourcemap.json`. The following
settings are configurable either way:
...
- `luau-lsp.sourcemap.rojoProjectFile`: What Rojo project file to use when there's no `.ovdrjm` (default: `default.project.json`)
...
If neither applies to your project layout, you can configure `luau-lsp.sourcemap.generatorCommand`
to run a custom generator. If your generator does not support file watching, enable
